...
//==============================================================================
	...
	//
	// IMPORTANT NOTE
	// there is no warranty, implied or otherwiseSchema::rename('$from', '$to'); with this software.
	//
	...
	function __construct($paper = 'a4', $orientation = 'portrait') {
		...
		if (!is_array($paper)) {
			...
			case 'FOLIO':{
					...
					break;}
			case 'PERMIT':{
					$size = array(0, 0, 370.00, 320.00);
					break;}
			case 'RECEIPT':{
					$size = array(0, 0, 612.00, 396.00);
					break;}
			case 'ORDERMEMO':{
					$size = array(0, 0, $this->C2Ppdf(13.5), $this->C2Ppdf(20.5));
					break;}
			case 'INVOICE':{
					$size = array(0, 0, $this->C2Ppdf(13.7), $this->C2Ppdf(20.4));
					break;}
			case 'SALESORDER':{
					$size = array(0, 0, $this->C2Ppdf(13.6), $this->C2Ppdf(21.4));
					break;}
			case 'SALESORDER_2':{
					$size = array(0, 0, $this->C2Ppdf(20.2), $this->C2Ppdf(14));
					break;}
			case 'DELIVERYR':{
					$size = array(0, 0, $this->C2Ppdf(21), $this->C2Ppdf(25.5));
					break;}
			case 'PROVISIONALRECEIPT':{
					$size = array(0, 0, $this->C2Ppdf(11), $this->C2Ppdf(13));
					break;}
			case 'SANYANG-DELIVERY':{
					$size = array(0, 0, $this->C2Ppdf(20.5), $this->C2Ppdf(26.3));
					break;}
			case 'SANYANG-INVOICE':{
					$size = array(0, 0, $this->C2Ppdf(20.5), $this->C2Ppdf(26));
					break;}
			case 'SANYANG-RETURN':{
					$size = array(0, 0, $this->C2Ppdf(16.5), $this->C2Ppdf(17.5));
					break;}
			case 'CHSM-OR':{
					$size = array(0, 0, $this->C2Ppdf(11.35), $this->C2Ppdf(18.6));
					break;}
			case 'CHSM-CV':{
					$size = array(0, 0, $this->C2Ppdf(21.6), $this->C2Ppdf(14));
					break;}
			case 'SWU_EXAM_PERMIT': {
				$size = array(0,0,$this->C2P(21.5),$this->C2P(8.2)); 
				break;}
			case 'SBCHECK':{
					$size = array(0, 0, $this->C2Ppdf(20.2), $this->C2Ppdf(7.6));
					break;}
			}
				...
				foreach ($this->ezPages as $pageNum => $id) {
					if (isset($tmp[$pageNum])) {
						if (is_array($tmp[$pageNum])) {
							...
							$info = $tmp[$pageNum];
							#dd($info['num'],$pageNum);
							$info['dnum'] = (int)$info['num'] - $pageNum;
...
// ------------------------------------------------------------------------------
	...
	function ezStream($options = '', $saveas = false, $filename = "") {
		$this->ezPRVTcleanUp();
		$this->stream($options, $saveas, $filename);
	}
...
// ------------------------------------------------------------------------------
	...
	function bullet($row, $col, $style = 'round') {
		$this->setLineStyle(3, $style, '', array(0, 1));
		$this->line($col, $row, $col + 2, $row);
		$this->setLineStyle(1);
	}
...
// ------------------------------------------------------------------------------
	...
	function C2Ppdf($fnVal) {
		$return = $fnVal * 28.32861;
		return $return;
	}
	...
	function addTextWrapShrink($x,$y,$width,$size,$text,$justification='left',$angle=0,$test=0){
	    $y_th = $y;
	    $caption = $text;
	    $fsize = $size;
	    if($caption&&$width>0){
	      # dd($fsize,$caption,$width);
	      while($this->getTextWidth($fsize,$caption)>$width){
	        $fsize -= 1;  
	      }
	      $fsize = $fsize<0?1:$fsize;
	      $capdisplay = $this->addTextWrap($x,$y_th,$width,$fsize,$caption,$justification,$angle,$test);
	      while(!empty($capdisplay)){
	          $y_th -= 10;
	          $capdisplay = $this->addTextWrap($x,$y_th,$width,$fsize,$capdisplay,$justification,$angle,$test);
	      } 
	    }
	}
	#### ADDITIONAL FUNCTION FOR CONVERING CENTIMETER TO PIXEL
	function C2P($fnVal){           #
	  $return = $fnVal * 28.32861;  #
	  return $return;               #
	}
