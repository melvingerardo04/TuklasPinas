...
*/
function ezPdf ($pageSize=array(0,0,612,792)){
...
function openFont($font){
  ...
  $this->addMessage('openFont: '.$font.' - '.$name);
    ...
    #dd($name);
    #dd($dir.'php_'.$name.'.afm' ,  file_exists($dir.'php_'.$name.'.afm'), $_SERVER['CONTEXT_DOCUMENT_ROOT'].'/public/assets/fonts/Calibri.afm', file_exists($_SERVER['DOCUMENT_ROOT'].'/public/assets/fonts/Calibri.afm'));
 # }
...
* the coordinates of the upper-right corner
*/
function rectangle($x1,$y1,$width,$height,$fill=FALSE,$r=0.9,$g=0.9,$b=0.9){
  $this->objects[$this->currentContents]['c'].="\n".sprintf('%.3f',$x1).' '.sprintf('%.3f',$y1).' '.sprintf('%.3f',$width).' '.sprintf('%.3f',$height).' re S';
  if($fill)
    {
	    $this->setColor($r,$g,$b);
	    $this->filledRectangle($x1+2,$y1+1,$width-3,$height-2);
	    $this->setColor(0,0,0);
    }
}
...
*/
function stream($options='',$saveas=false,$filename='file.pdf'){
  ...
  $fileName = (isset($options['Content-Disposition'])?$options['Content-Disposition']:$filename);
   // header("Content-Disposition: $KahitAno; filename=".$fileName);
  ...
  if (!is_array($options)){
    if ($family=='init'){
      ...
      $this->fontFamilies['arial.afm']=array(
         'b'=>'arial-Bold.afm'
        ,'i'=>'arial-Italic.afm'
        ,'bi'=>'arial-Bold-Italic.afm'
        ,'ib'=>'arial-Bold-Italic.afm'
      );

      $this->fontFamilies['Calibri.afm']=array(
         'b'=>'Calibri-Bold.afm'
        ,'i'=>'Calibri-Italic.afm'
        ,'bi'=>'Calibri-BoldItalic.afm'
        ,'ib'=>'Calibri-BoldItalic.afm'
      );
    }
  } else {
